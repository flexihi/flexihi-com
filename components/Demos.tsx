...
const DemoCard = memo(function DemoCard({ demo, locale, t }: DemoCardProps) {
  return (
    <article className='group border border-gray-200 rounded-xl px-4 py-4 sm:px-6 sm:py-5 lg:px-11 lg:py-6 hover:border-primary-light hover:bg-primary-lightest hover:shadow-sm interactive-hover flex flex-col items-center text-center sm:items-start sm:text-left h-full'>
      <Image
        src={demo.icon}
        alt={`${demo.title} icon`}
        width={48}
      ...
      <div className='mt-2 sm:mt-3 sm:hidden mb-4'>
        <div className='flex flex-wrap justify-center gap-x-2 gap-y-1'>
          {demo.list.map((item, index) => (
            <span
              key={index}
              className='text-text-body text-sm leading-5 flex items-center'
            >
              {item}
      ...
      <ul className='mt-2 sm:mt-3 hidden sm:block mb-4'>
        {demo.list.map((item, index) => (
          <li
            key={index}
            className='text-text-body text-sm sm:text-base leading-5 sm:leading-6'
          >
            {item}
          </li>
        ))}
      ...
      <button
        className='mt-auto w-full sm:w-auto text-secondary border border-secondary border-opacity-50 hover:shadow-md rounded-xl px-5 py-2 flex gap-2 items-center justify-center sm:justify-start interactive-hover touch-target text-[15px]'
        type='button'
